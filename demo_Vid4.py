...
def main(cfg):
    ...
    ckpt = torch.load('./log/reciprocal_ssim_BI_x4_iter8000.pth')
    ...
    for idx_iter, (LR_y_cube, SR_cb, SR_cr) in enumerate(test_loader):
        ...
        if not os.path.exists('results/calendar_re_8000'):
            os.mkdir('results/calendar_re_8000')
        SR_rgb.save('results/calendar_re_8000'+'/sr_'+ str(idx_iter+2).rjust(2,'0') + '.png')
